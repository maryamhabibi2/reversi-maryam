 ...
 */
    ...
    socket.on('invite', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
            response = {
                result: 'fail',
                message: 'the user that was invited is does not have a name registered'
            }
            socket.emit('invite_response', response);
            serverLog('invite command failed', JSON.stringify(response));
            return;
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof room == 'undefined') || (room === null) || (room === "")) {
            ...
            socket.emit('game_start_response', response);
            serverLog('game_start', JSON.stringify(response));
            return;
        }
        if ((typeof username == 'undefined') || (username === null) || (username === "")) {
            response = {
 ...
 */
    ...
    socket.on('play_token', (payload) => {
        ...
        if(color !== game.whose_turn) {
            ...
            socket.emit('play_token_response',response);
            serverLog('play token command failed', JSON.stringify(response));
            return;
        }
...
function adjacent_support(who,dr,dc,r,c,board){
    ...
    /* Check to make sure that there is space for a matching color to capture tokerns */
        ...
        if (sockets.size >=1) {
            ...
            if((games[game_id].player_white.socket !=first) &&
                ...
                if (games[game_id].player_white.socket === "") {
                    ...
                    console.log("White is assgined to: " + first);
                    ...
                    console.log("Black is assgined to: " + first);
                    games[game_id].player_black.socket = first;
